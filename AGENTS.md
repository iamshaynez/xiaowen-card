...
- **全屏高 scroll-view 内原生 input 聚焦会文字错位**：`.panel-scroll` 撑满视口高时，input/textarea 聚焦弹键盘，微信对原生输入层的焦点定位计算出错，已输入文字会上浮错位（落款输入框紧邻吸顶预览区，错位文字直接飘进卡片渲染区）。解法是聚焦期禁滚：输入框 `bindfocus/bindblur` 维护 `inputFocus`，`scroll-y="{{!inputFocus}}"`（`onFieldFocus`/`onFieldBlur`），失焦恢复滚动。
