...
- **月白笺 moon**：月白底 `#E4ECEC`、深灰蓝字 `#2E3F46`
- **黛蓝卡 indigo**：黛蓝底 `#232D36`、浅灰青字 `#D9E2E2`
- **茶烟笺 tea**：茶白底 `#EFE9DD`、褐灰字 `#453B31`
- **竹青笺 bamboo**：竹青底 `#E3E8DE`、墨绿灰字 `#37473B`

后四种只换配色主题，复用横排版式（金句 / 长文两种模式自动支持）。

卡片结构：Banner → 发丝线 → 正文（金句模式带红色「」引号、居中大字；长文模式左对齐、逐字换行）→ 固定落款 + 朱文印章（取落款首行首字；上传 Logo 时由 Logo 替代印章位置）。

落款支持两行（`\n` 分隔，最多取两行）：

- **横排**：单行保持「—— 署名」；两行为「上行小字三级灰（日期/地点）+ 下行署名次级灰」，右对齐叠排于印章左侧、垂直居中，无破折号前缀。
- **竖排**：两行渲染为两竖列，第一行在右、第二行在左（传统款式），列底对齐，印章居中于列组下方。
